...
class mainwindow(QMainWindow):
    ...
    def ui_event(self):
        ...
        self.ui.action_importxls.triggered.connect(self.show_bs)
        self.ui.OpenExcel.clicked.connect(self.import_seq)
    ...
    def show_bs(self):
        if self.ui.label_19.text()!="補數檔案":
            QMessageBox.information(self, "補數信息!", "補數數量: "+str(len(self.seqlist))+" 個;"+chr(10)+"補數編號:"+chr(10)+str(self.seqlist))
    ...
    def import_seq(self):
        ...
        if filelist!=[]:
            ...
            print(xls.columns.values[0]=='開始編號')
            if xls.columns.values[0]=='開始編號':
                seqlist_w = ()
                for r,v in xls.iterrows():
                    if pd.isna(v[1]):
                        seqlist_w = seqlist_w + (int(v[0]),)
                    else:

                        for range1 in range(int(v.values[0]),int(v.values[1])+1):
                            seqlist_w = seqlist_w + (range1,)
            else:
                va=xls.values[0][0]
                ...
                seqlist_w = ()
                for i in xls.values:
                    seqlist_w = seqlist_w + (int(i[0][possiton:]),)

            #去重復及排序.
            set_data = set(seqlist_w)   #元组转换为集合去重複
            list_data = list(set_data)  #集合轉列表
            list_data.sort()            #列表sort 排序
            self.seqlist = tuple(list_data)     #列表轉元組
            # 去重復及排序.
            ...
            print('原數據:',seqlist_w)
            print('去重復及排序后數據:',self.seqlist)
            self.ui.label_19.setText("補數檔案: "+filelist[0]+"; 數量: "+str(len(seqlist_w))+" 個")
            scanfold, select_filename = os.path.split(filelist[0])
            filename = os.path.splitext(select_filename)[0]
            if os.path.exists("D:/PDF/"+filename+".xls") or os.path.exists("D:/PDF/"+filename+".xlsx"):
                QMessageBox.information(self, "錯誤提示!", '文件名已存在: '+filename)
                return
            else:
                shutil.move(filelist[0],"D:/PDF")
        else:
            print("文件名空")
    ...
    def read_lotus_LaserRoom(self,BsKey):
        ...
        sort1 = sorted(seqlist1, key=lambda x: x[1])
        sort2 = ()
        for s1 in sort1:
    ...
    def PDFLicense(self,company,year):
        b = company.encode()
        # print(b)
        passkey = ""
        for en1 in b:
    ...
    def load_data(self):
        ...
        with open("config.json", encoding="utf-8-sig") as f:
            DEFAULT = json.load(f)["DEFAULT"]

        # config = configparser.ConfigParser()
        # config.read("./config.ini",encoding="UTF-8")

        self.db_path = DEFAULT["數據庫路徑"]
        # 注册码
        PDF_company = DEFAULT['公司']
        PDF_License_input = DEFAULT['注册码_补数']
        PDF_License=self.PDFLicense(PDF_company,datetime.date.today().year)
        if PDF_License_input!=PDF_License:
            if datetime.date.today().month==1:
                PDF_License2=self.PDFLicense(PDF_company,datetime.date.today().year-1)
                if PDF_License_input == PDF_License2:
                    QMessageBox.information(self, "注册码到期提示!", "注册码到期, 请尽快更新注册码, 否则很快无法使用!")
                    self.setWindowTitle("PDF补数-注册码到期, 请尽快更新注册码, 否则很快无法使用!")
                else:
                    QMessageBox.information(self, "注册码错误提示3!", "注册码错误3, PDF补数无法使用!")
                    self.setWindowTitle("PDF补数无法使用!")
                    self.setEnabled(False)

            else:
                QMessageBox.information(self, "注册码错误提示2!", "注册码错误2, PDF补数无法使用!")
                self.setWindowTitle("PDF补数无法使用!")
                self.setEnabled(False)
        # 注册码


        self.dirpath = DEFAULT['补数默认路径']
        self.Lotus_server = DEFAULT['Lotus_server']
        self.outputfile = DEFAULT['輸出路徑']
        fontcolor = DEFAULT['忽略字休顏色']
        self.fontcolor = [int(i) for i in fontcolor.split(';')]
        # print("outputfile:", outputfile)
        # self.ui.comboBox1.setCurrentText(DEFAULT['編號次序'])
        # self.ui.comboBox2.setCurrentText(DEFAULT['PDF输出方向'])
        # self.ui.comboBox3.setCurrentText(DEFAULT['出血位'])
        self.ui.lineEdit_r1c1.setText(DEFAULT['橫排個數'])
        self.ui.lineEdit_r1c2.setText(DEFAULT['橫刀位(寸)'])
        self.ui.lineEdit_r1c3.setText(DEFAULT['左右出血位(寸)'])
        self.ui.lineEdit_r1c4.setText(DEFAULT['上下出血位(寸)'])
        self.ui.lineEdit_r1c5.setText(DEFAULT['右邊增減'])
        self.ui.lineEdit_r1c6.setText(DEFAULT['下邊增減'])
        self.ui.lineEdit_r2c1.setText(DEFAULT['豎排個數'])
        self.ui.lineEdit_r2c2.setText(DEFAULT['豎刀位(寸)'])
        # self.ui.lineEdit_r2c3.setText(DEFAULT['實際成品長'])
        # self.ui.lineEdit_r2c4.setText(DEFAULT['實際成品寬'])
        self.ui.lineEdit_r3c3.setText(DEFAULT['水平位置'])
        self.ui.lineEdit_r3c4.setText(DEFAULT['垂直位置'])
        self.ui.lineEdit_r3c1.setText(DEFAULT['一箱張數'])
        self.ui.lineEdit_r3c2.setText(DEFAULT['一個PDF檔頁數'])
    ...
    def get_pdf_info(self, filelist):
        ...
        for row_num,fl in enumerate(filelist):
            ...
            if re.search('_job', fl) is not None:
                ...
                model.setItem(row_num, 0, QStandardItem(os.path.splitext(filename)[0]))
    ...
    def getPDFinfo(self,FileName):
        PDFdata = sqlite3.connect(self.db_path+"PDFdata.db")
        cursor = PDFdata.cursor()
        cursor.execute("SELECT * FROM `JobInfo` where FileName=?", (FileName,))
    ...
    def Open_File2(self):
        ...
        if filelist!=[]:
            ...
            Folder, FileName2 = os.path.split(filelist[0])
            FileName, file_extension = os.path.splitext(FileName2)
            model = self.get_pdf_info(filelist)
            self.ui.open_tableView1.setModel(model)
            self.ui.open_tableView1.resizeColumnToContents(0)
            self.ui.open_tableView1.resizeColumnToContents(1)
            ...
            row=self.getPDFinfo(FileName)
            if row!=None:

                print(row[3],row[4])

                self.start_seq=row[34]+1
                dirpath=os.path.split(filelist[0])[0]+"/"
                model = self.get_pdf_info(filelist)
                self.ui.open_tableView1.setModel(model)
                self.ui.open_tableView1.resizeColumnToContents(0)
                ...
                self.ui.comboBox3.setCurrentText(row[4])
                self.ui.comboBox_rotate.setCurrentText(str(row[31]))
                self.ui.lineEdit_r1c1.setText(row[5])
                self.ui.lineEdit_r1c2.setText(row[6])
                self.ui.lineEdit_r1c3.setText(row[7])
                self.ui.lineEdit_r2c1.setText(row[8])
                self.ui.lineEdit_r2c2.setText(row[9])
                self.ui.lineEdit_r1c4.setText(row[10])
                self.ui.lineEdit_r3c1.setText(row[11])
                self.ui.lineEdit_r3c2.setText(row[12])
                self.ui.lineEdit_Repeat.setText(str(row[33]))
                self.ui.lineEdit_StartSeq.setText(str(row[34]+1))
