...
class mainwindow(QMainWindow):
        ...
        if PDF_License_input!=PDF_License:
            if datetime.date.today().month==1:
                ...
                if PDF_License_input == PDF_License2:
                    ...
                    self.setWindowTitle("PDF排版-注册码到期, 请尽快更新注册码, 否则很快无法使用!")
                else:
                    QMessageBox.information(self, "注册码错误提示3!", "注册码错误3, PDF排版无法使用!")
                    self.setWindowTitle("PDF排版无法使用!")
                    self.setEnabled(False)
                    # sys.exit(app.exec_())

            else:
                QMessageBox.information(self, "注册码错误提示2!", "注册码错误2, PDF排版无法使用!")
                self.setWindowTitle("PDF排版无法使用!")
                self.setEnabled(False)
                # sys.exit(app.exec_())
        # else:
        #     QMessageBox.information(self, "注册码正确!", "注册码正确!")

        outputfile = DEFAULT['輸出路徑']
        self.Lotus_server = DEFAULT['Lotus_server']
        print("outputfile:", outputfile)
        self.Processing_department = DEFAULT['處理部門']
    ...
    def open_folder(self):
        ...
        for f in file:
            ...
            r2 = re2.findall(f)
            r3 = re3.findall(f)
            if not r1 and not r2 and not r3 and f.lower().endswith(".pdf") :    #增加and f.lower().endswith(".pdf"), 把文件夹去掉
...
def write_PDFdata(path, print_type, bleeding_type, e1, e2, e3, e4, e5, e6, e7, e8, data_count,page_count_first,page_count_last,width,height,width_total,height_total,direction_type,r2c3,r2c4,r3c3,r3c4,r1c5,r1c6,check_Double_sided,addart,check_addblank,check_addblank2,PDF_rotate,addpagenum,times_input,StartSeq):

    Folder, FileName2 = os.path.split(path)
    FileName, file_extension = os.path.splitext(FileName2)
    PDFdata = sqlite3.connect(ui_mainwindow.db_path+"PDFdata.db")

    cursor = PDFdata.cursor()
    cursor.execute("SELECT FileName FROM `JobInfo` where FileName=?",(FileName,))
    Check_FileName=cursor.fetchall()
    if len(Check_FileName)>0:
        cursor.execute("Delete FROM `JobInfo` where FileName=?", (FileName,))
        PDFdata.commit()

    cursor.execute("INSERT INTO `JobInfo` VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (FileName, Folder, path, print_type, bleeding_type, e1, e2, e3, e4, e5, e6, e7, e8, data_count,page_count_first,page_count_last,width,height,width_total,height_total,direction_type,r2c3,r2c4,r3c3,r3c4,r1c5,r1c6,check_Double_sided,addart,check_addblank,check_addblank2,PDF_rotate,addpagenum,times_input,StartSeq))
    PDFdata.commit()
    cursor.close()
    PDFdata.close()
...
def combine(list1):
    ...
    write_PDFdata(path, print_type, bleeding_type, e1, e2, e3, e4, e5, e6, e7, e8, data_count, page_count_first, page_count_last, width, height, width_total, height_total, direction_type, r2c3, r2c4, r3c3, r3c4, r1c5, r1c6, check_Double_sided, addart, check_addblank, check_addblank2, PDF_rotate, addpagenum, times_input, StartSeq)
    ...
    # Hjob = "1" + fx[0].replace("-", "")[2:8]  #改为40工单号
    # Hjob = fx[0]    #不改工单名
    # Hlot = fx[1][1:]
    # Hitem = fx[2]
    # fx3=''
    ...
    PDF_filename_output = str(PDF_filename).replace(" ","_")
